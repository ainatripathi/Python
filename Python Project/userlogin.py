...
class Profile(tk.Frame):
    ...
    def update_user(self):
        ...
        cursor=conn.cursor()
        try:
            cursor.execute("UPDATE data SET username=%s WHERE id=%s",(self.new_user,self.controller.id))
            conn.commit()
            self.controller.username=new_user
            self.lbl.config(text=f"Welcome {new_user}!")
